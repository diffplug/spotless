...
public final class BundleController implements StaticBundleContext {
	...
	public BundleController(ServiceCollection services, BundleActivator[] frameworkActivators) throws BundleException {
		...
		properties.put(org.eclipse.core.internal.runtime.FindSupport.PROP_NL, ""); //Used to set-up an internal, unused member
		...
		this.services.add(org.osgi.service.packageadmin.PackageAdmin.class, new EclipseBundleLookup(bundles));
		this.services.initialize(systemBundle, properties);

		//The framework activators requiring org.eclipse.osgi bundle.
		//But since the osgi
		bundles.add(new SystemBundle(ECLIPSE_LAUNCHER_SYMBOLIC_NAME, Bundle.ACTIVE, this));
		for (BundleActivator frameworkActivator : frameworkActivators) {
			try {
				frameworkActivator.start(this);
			} catch (Exception e) {
				throw new BundleException(String.format("Failed to start %s", frameworkActivator.getClass().getName()), BundleException.ACTIVATOR_ERROR, e);
			}
		}
	}
	...
	public Bundle getBundle(String location) {
		if (location == Constants.SYSTEM_BUNDLE_LOCATION) {
			return systemBundle;
		}
		return null;
	}
	...
	private static class SystemBundle implements StaticBundle, TemporaryBundle {
		...
		/** System bundle with a custom fixed state ad symbolic name */
		...
		private SystemBundle(String symbolicName, BundleActivator activator, int fixedState, BundleContext bundleController) throws BundleException {
			...
			String objPath = bundleObj.getClass().getName();
			metaInfFatJarResourcePath = objPath.substring(0, objPath.lastIndexOf('.'));
		...
		private URL tryFatJarMetaInf(String path) {
			return getResource(metaInfFatJarResourcePath + "/" + path);
		}
