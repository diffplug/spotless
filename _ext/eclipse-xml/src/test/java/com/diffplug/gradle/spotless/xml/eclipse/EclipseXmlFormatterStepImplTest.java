...
public class EclipseXmlFormatterStepImplTest {
	...
	@Test
	public void unsupportedConfiguration() throws Throwable {
		String output = format(TEST_DATA.input("xml_space.xml"), config -> {
			config.setProperty(CMDOCUMENT_GLOBAL_CACHE_ENABLED, Boolean.toString(true));
		});
		assertEquals("No exception thrown due to enabled global cache. But unsupported config leads to incorrect formatting.",
				TEST_DATA.expected("xml_space.xml"), output);
	}
	...
	public void xsdNotFound() throws Throwable {
		...
		assertEquals("Unresolved XSD/DTD not silentlý ignored.",
	...
	public void catalogLookup() throws Throwable {
		String output = format(TEST_DATA.input("xsd_not_found.xml"), config -> {
			config.setProperty(
					EclipseXmlFormatterPreferenceNames.USER_CATALOG,
